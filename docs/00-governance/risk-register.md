...
| R-15 | Many GPUs on one consumer board exhaust 64-bit MMIO / BAR space or bus numbers; bricks beyond 2–4 do not enumerate (CR-006) | 3 | 3 | 9 | Above-4G decoding + Resizable BAR on; check L13; qualify 4 bricks on the chosen host before committing to a hub | Hardware Eng. | Open |
| R-16 | First spin of the Gen4 backplane PCB fails signal integrity (CR-007) | 3 | 3 | 9 | Phase A on an off-the-shelf switch board; SI simulation; test coupons; experienced board house | Hardware Eng. | Open |
| R-17 | GeForce P2P needs an unofficial Linux driver patch that can crash or corrupt output on some platforms | 3 | 3 | 9 | Host all-reduce stays the default; P2P opt-in; `astra auto` measures and validates output | Solution Architect | Open |
