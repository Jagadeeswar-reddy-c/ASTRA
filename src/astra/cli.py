"""``astra`` command-line interface.
    ...
    astra auto                  detect every GPU, configure, choose + download the best model,
                                launch, benchmark, validate, open the console (no input needed)
...
from astra import __version__, asbuilt, pool
...
def _topology_config(inv: Inventory, cfg: AstraConfig, uuids: list[str]) -> list[str]:
    return asbuilt.topology_config(inv, cfg, uuids)
...
def cmd_auto(args: argparse.Namespace, cfg: AstraConfig) -> int:
    from astra.auto import AutoOptions, default_lab, run

    opts = AutoOptions(
        lab=Path(args.lab_dir) if args.lab_dir else default_lab(),
        context=args.context or cfg.planner.default_context,
        model=args.model,
        quant=args.quant,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser(
        "auto",
        help="detect all GPUs, configure, pick the best model, download, launch, verify",
    )
    p.add_argument("--lab-dir", help="where llama.cpp, models and results go (~/astra-lab)")
    p.add_argument("--context", type=int, help="context length to plan for (default 8192)")
    p.add_argument("--model", choices=sorted(CATALOG), help="force a catalog model")
    p.add_argument("--quant", choices=["q8_0", "q6_k", "q5_k_m", "q4_k_m"], help="force a quant")
    p.add_argument("--min-tps", type=float, default=8.0, help="skip plans slower than this")
    p.add_argument("--binary", help="llama-server to use instead of auto-download")
    p.add_argument("--port", type=int, default=8080, help="engine port")
    p.add_argument("--ui-port", type=int, default=9838, help="console port")
